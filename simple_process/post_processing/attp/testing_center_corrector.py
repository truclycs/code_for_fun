...
class TestingCenterCorrector:
    ...
    def get_idx_line(self, s):
        ...
        if len(indexes) > 1:
            for idx, cnt in zip(idx_lines, counts):
                print(idx, cnt, len(no_accent_vietnamese(self.testing_center_list[idx]).split(' ')), self.testing_center_list[idx])
                if cnt == len(self.testing_center_list[idx]):
                    print(self.testing_center_list[idx])
...
if __name__ == '__main__':
    ...
    testcases = ['BỘ Y TẾ |VIỆN PASTEUR NHA TRANG |TRUNG TÂM KIỂM NGHIỆM ATVSTP KHU VỰC MIÊN TRUNG']
